...
$result = [ordered]@{
  ...
  next_action = $(if ($ok) { 'Back on Cockpit, run npm.cmd run codexa:access, npm.cmd run codexa:remote-proof, npm.cmd run codexa:alert:popup, npm.cmd run codexa:watch, and npm.cmd run ops:green.' } else { 'Fix the first required failure in steps, then rerun this start-here launcher.' })
