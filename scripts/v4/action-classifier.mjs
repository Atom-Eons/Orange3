...
const SAFE_RULES = [
  ...
  { id: "orangebox_proof_script", re: /\bnpm(\.cmd)?\s+run\s+(backend:proof|ops:readiness|health:report|project:report|reality:watch|harness:benchmark|action:doctor|package-script-doctor|codexa:alert|mcp:doctor|skills:lifecycle)\b/i },
