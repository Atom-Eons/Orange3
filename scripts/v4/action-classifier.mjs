...
const SAFE_RULES = [
  ...
  { id: "orangebox_proof_script", re: /\bnpm(\.cmd)?\s+run\s+(backend:proof|ops:readiness|ops:green|ops:gaps|health:report|project:report|reality:watch|harness:benchmark|research:radar|assurance:doctor|tool:ergonomics|checkmate:doctor|signal:hygiene|session:spine|feature:proof|final:verify|final:zip|action:doctor|package-script-doctor|codexa:alert|codexa:alert:popup|codexa:access|codexa:remote-proof|codexa:watch|codexa:watch:popup|codexa:smb-stage|codexa:handoff|mcp:doctor|ipi:doctor|memory:doctor|skills:lifecycle|model:lane-eval|model:inventory)\b/i },
...
export const ACTION_CLASSIFIER_FIXTURES = [
  ...
  { name: "final-verify", command: "npm.cmd run final:verify", expect: { disposition: "allow", blocked: false } },
  { name: "final-zip", command: "npm.cmd run final:zip", expect: { disposition: "allow", blocked: false } },
